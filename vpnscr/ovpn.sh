...
sudo apt update
sudo apt install openvpn easy-rsa curl jq -y
...
serv_cert() {
    ...
    if [[ -f "$servcert" ]]; then
        ...
        cd /home/$user/easy-rsa && source /home/$user/easy-rsa/easyrsa gen-req server nopass
        ...
        $sed '/-----BEGIN CERTIFICATE-----/,$!d' /etc/openvpn/server/server.crt
...
tls_crypt() {
    ...
    sed -i '/-----BEGIN OpenVPN Static key V1-----/,$!d' /home/$user/client-configs/keys/ta.key
}
tls_crypt
...
client_cert() {
    ...
    if [[ -z $option ]]; then
        ...
        read client_name
        cd /home/$user/easy-rsa && source /home/$user/easy-rsa/easyrsa gen-req $client_name nopass
        ...
        $sed '/-----BEGIN CERTIFICATE-----/,$!d' /home/$user/client-configs/keys/$client_name.crt
...
#Include logging
$sh "echo 'log-append /var/log/openvpn.log' >> /etc/openvpn/server/server.conf"
